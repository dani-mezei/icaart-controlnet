...
def log_validation(
    vae, text_encoder, tokenizer, unet, controlnet, args, accelerator, weight_dtype, step, is_final_validation=False, negative_prompt=None
):
    ...
    # If more then `NUM_OF_VALIDATION_SAMPLES` validation images, select only `NUM_OF_VALIDATION_SAMPLES` number of samples
    if len(validation_images) > NUM_OF_VALIDATION_SAMPLES:
        seeded_random = random.Random(args.seed)

        # Zip the prompts and images together
        zipped = list(zip(validation_prompts, validation_images))

        # Sample
        zipped = seeded_random.sample(zipped, NUM_OF_VALIDATION_SAMPLES)

        # Unzip
        validation_prompts, validation_images = zip(*zipped)
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The negative prompt that is passed to the Stable Diffusion Pipeline"
        )
    )
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    def preprocess_train(examples):
        ...
        selected_weather_transform = None
        condition_name = ""


        no_condition_applied = transforms.Lambda(lambda x: x)
